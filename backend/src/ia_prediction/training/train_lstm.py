...
_SRC_ROOT = _IA_PRED_ROOT.parent
if str(_SRC_ROOT) not in sys.path:
    sys.path.insert(0, str(_SRC_ROOT))

DEFAULT_SEQUENCES = Path(os.getenv(
    "SEQUENCES_PATH",
    str(_SRC_ROOT / "ia_prediction" / "training" / "data" / "processed" / "sequences.npy"),
))
DEFAULT_LABELS = Path(os.getenv(
    "LABELS_PATH",
    str(_SRC_ROOT / "ia_prediction" / "training" / "data" / "processed" / "labels.npy"),
))
DEFAULT_MODEL_OUT = Path(os.getenv(
    "MODEL_SAVE_PATH",
    str(_SRC_ROOT / "ia_prediction" / "models" / "lstm_predictor.pt"),
))
...
def _resolve_path(env_key: str, default: Path) -> Path:
    ...
    if raw:
        p = Path(raw)
        return p if p.is_absolute() else _SRC_ROOT / p
    cfg_path = Path(getattr(config, env_key, str(default)))
    return cfg_path if cfg_path.is_absolute() else Path(cfg_path)
...
def _load_data() -> tuple[np.ndarray, np.ndarray, Path]:
    ...
    if not seq_path.exists() or not lbl_path.exists():
        ...
        print("  backend/src/ia_prediction/training/data/processed/sequences.npy")
        print("  backend/src/ia_prediction/training/data/processed/labels.npy")
        print("")
        print("Run first: python backend/src/ia_prediction/training/download_dataset.py")
        print("Or: bash backend/src/ia_prediction/fix_and_train.sh")
...
def main() -> None:
    try:
        ...
        print("Run: bash backend/src/ia_prediction/fix_and_train.sh")
        sys.exit(1)
